...
# CONTRIBUTORS
printf "#### Contributors\n\n" > CONTRIBUTORS.md
git log | git shortlog $LAST_VERSION.. --numbered --summary -e |while read a rest || [ -n "$rest" ]; do echo "- $rest";done >> CONTRIBUTORS.md

# Update AUTHORS
git log | git shortlog --numbered --summary -e |while read a rest || [ -n "$rest" ]; do echo $rest;done > AUTHORS
